...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM Prerequisites p WHERE p.course = NEW.course AND 
        NOT EXISTS (SELECT 1 FROM Taken t WHERE t.student = NEW.student 
...
BEGIN
    ...
    IF OLD.status = 'registered' THEN
        ...
        IF EXISTS (SELECT 1 FROM LimitedCourses WHERE code = OLD.course) THEN
            ...
            SELECT COUNT(*) INTO current_registered FROM Registered WHERE course = OLD.course;
            ...
            IF current_registered < course_capacity THEN
                ...
                AND NOT EXISTS ( SELECT 1 FROM Prerequisites p WHERE p.course = OLD.course
                    AND NOT EXISTS ( SELECT 1 FROM Taken t WHERE t.student = W.student 
                        AND t.course = p.prerequisite 
    ...
    ELSIF OLD.status = 'waiting' THEN
        ...
        SET position = position - 1
        WHERE course = OLD.course
        AND position > (SELECT position FROM WaitingList  WHERE student = OLD.student 
            AND course = OLD.course
        );
    END IF;
